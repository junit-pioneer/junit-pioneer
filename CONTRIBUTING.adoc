...
* first of all, the https://docs.junit.org/current/user-guide/#extensions[JUnit 5 User Guide]
...
1. once before/after all tests (meaning it "brackets" the test class, typically via https://docs.junit.org/current/api/org.junit.jupiter.api/org/junit/jupiter/api/extension/BeforeAllCallback.html[`BeforeAllCallback`] / https://docs.junit.org/current/api/org.junit.jupiter.api/org/junit/jupiter/api/extension/AfterAllCallback.html[`AfterAllCallback`]) or
2. once before/after each test (meaning it "brackets" each test method, typically via https://docs.junit.org/current/api/org.junit.jupiter.api/org/junit/jupiter/api/extension/BeforeEachCallback.html[`BeforeEachCallback`] / https://docs.junit.org/current/api/org.junit.jupiter.api/org/junit/jupiter/api/extension/AfterEachCallback.html[`AfterEachCallback`])?
...
Interacting with https://docs.junit.org/current/user-guide/#extensions-keeping-state[Jupiter's extension `Store`] requires a `Namespace` instance.
...
To that end it is necessary to understand https://docs.junit.org/current/user-guide/#writing-tests-parallel-execution[JUnit Jupiter's parallel execution], particularly https://docs.junit.org/current/user-guide/#writing-tests-parallel-execution-synchronization[the synchronization mechanisms it offers]: `@Execution` and `@ResourceLock`.
...
JUnit Pioneer uses https://jreleaser.org/[JReleaser] and https://github.com/features/actions/[GitHub Actions] to automate the release process.
We don't release on every commit to `main`.
...
GitHub Actions will then tell Gradle/JReleaser to do their thing.
...
Under the hood, the `release` job runs `./gradlew publish jreleaserDeploy jreleaserRelease`:

* `publish` stages the artifacts in `build/staging-deploy`
* `jreleaserDeploy` signs them (PGP) and uploads + releases them through the https://central.sonatype.org/register/central-portal/[Sonatype Central Portal] Publisher API (the legacy OSSRH / `oss.sonatype.org` was shut down on 2025-06-30)
* `jreleaserRelease` creates the `v<version>` Git tag and the GitHub release, with notes generated by GitHub (merged PRs + new contributors)

The job needs these repository secrets:

* `JUNIT_PIONEER_PGP_PUBLIC_KEY` / `JUNIT_PIONEER_PGP_SECRET_KEY` / `JUNIT_PIONEER_PGP_PASSWORD` - the ASCII-armored PGP public key, secret key, and its passphrase used for signing (export the public key with `gpg --armor --export <key-id>`)
* `CENTRAL_PORTAL_USERNAME` / `CENTRAL_PORTAL_PASSWORD` - a https://central.sonatype.com/account[Central Portal user token] (these replace the old `NEXUS_TOKEN_*` secrets)
...
**A note on the version**: we pick it by hand and pass it to the _Release build_ action - there is no auto-versioning.
JReleaser resolves the previous tag from the Git history on its own (which is why the release job checks out with full history).
