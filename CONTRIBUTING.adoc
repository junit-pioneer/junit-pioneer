...
Optional dependencies are declared `compileOnly` on the main source set (so Pioneer can compile against them without requiring them at run time) and `testImplementation` on the test source set (so Pioneer's own tests exercise the integration).
