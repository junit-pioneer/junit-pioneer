...
public class DisableTest extends AbstractPioneerTestEngineTests {
	...
	void testSingleWithReasoning() {
		ExecutionEventRecorder recorder = executeTests(DisableTestCases.class,
			"disableSingleWithReason(int,java.lang.String)");
	...
	void testContains() {
		ExecutionEventRecorder recorder = executeTests(DisableTestCases.class,
			"disableContains(java.lang.String,java.lang.String)");
	...
	static class DisableTestCases {
		...
		public void failedDisable(int number, String text) {
			Disable.when(number, text).is(1, "a", "c");
		...
		public void disableSingle(int number, String text) {
			Disable.when(number, text).is(1, "a");
			assertThat(Arrays.asList(number, text)).isNotEqualTo(Arrays.asList(1, "a"));
		}
		...
		public void disableSingleWithReason(int number, String text) {
			Disable.when(number, text).is(1, "a").becauseOf("single issue");
			assertThat(Arrays.asList(number, text)).isNotEqualTo(Arrays.asList(1, "a"));
		}
		...
		public void disableContains(String key, String value) {
			Disable.when(key, value).contains("a").becauseOf("should not matter if key or value");
		...
		public void disableCompound(int number, String text) {
			Disable.when(number, text).contains(1).is(3, "a").is(2, "b").becauseOf("something that should be fixed");
