...
class DisabledIfNotReachableExtension implements ExecutionCondition {
	...
	public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
		...
		return enabled("No @DisabledIfNotReachable annotation found.");
			...
			DisabledIfNotReachableConfiguration config = readConfigurationFromAnnotation(optAnnotation.get());

		boolean reachable = pingUrl(config.getUrl(), config.getTimeOut());
		...
		if (reachable) {
			...
			return disabled(format(
					...
					context.getUniqueId(), config.getUrl(), config.getTimeOut()));
		}
     ...
     *
     * (From <a href="https://stackoverflow.com/users/157882/balusc">BalusC</a> on StackOverflow:
