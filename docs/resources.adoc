...
When using `@New` resources, the tests using them will continue to run in parallel.

When using `@Shared` resources, tests using them will be forced to run *sequentially*, even if parallel execution has been enabled.
