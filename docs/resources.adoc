...
This extension will call the constructor of the factory when the first test that has one or more parameters annotated with an e.g. `@New(InMemoryDirectory.class)` or `@Shared(factory = InMemoryDirectory.class)` annotation is about to run.

[NOTE]
====

This class must have a parameter-less constructor to be able to be instantiated by this extension.

If there isn't such a constructor present, then the extension will throw an exception.
...
This extension will call `close()` when all tests are finished.

Overriding `close()` is optional. It will do nothing by default.
...
This extension will call this method when a test with a `@New`- or `@Shared`-annotated parameter is about to run. The extension will then populate the parameter with the contents of this resource.
...
For a `@New` parameter, `close()` will be called and the resource will be made eligible for garbage collection immediately after the test.

For a `@Shared` parameter, `close()` will be called when all tests are finished. The resource will be cached in-memory until then.
...
These will be passed to the factory's `ResourceFactory.create()` method as a list when a resource is in the process of being created.

You have full control over what these arguments mean and how they are used. You should document these details in your resource factory's JavaDocs.

////
TODO: Document the above for TemporaryDirectory in its JavaDocs.
////
...
Tests with `@New` resources will continue to run in parallel.

Tests with `@Shared` resources will be forced to run *sequentially*, even if parallel execution has been enabled.
This is because resources may be _mutable_, and if the tests were allowed to run in parallel, then they could mutate the resources in a non-deterministic way.
