...
NOTE: The CartesianTest extension has undergone significant changes.
This page covers the old version (in package `org.junitpioneer.jupiter`), which was deprecated in 1.6.0 and removed in 2.0.
The new version (in package `org.junitpioneer.jupiter.cartesian`) has link:/docs/cartesian-product[its own documentation page].
