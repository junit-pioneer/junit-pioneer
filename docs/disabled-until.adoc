...
A fixed execution date can be configured to make reruns reproducible, as described link:disabled-until.adoc#reproducible_builds[below].
...
To make a build reproducible, set the `org.junitpioneer.jupiter.disableduntil.executiondate` https://docs.junit.org/current/overview.html#running-tests-config-params[configuration parameter] to the build's execution date in ISO 8601 format.
