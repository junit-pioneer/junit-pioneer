...
The test has been fully implemented, but the production code has not been implemented yet and throws an `UnsupportedOperationException` to indicate this.
Once the production code has been implemented the test method will fail if the code were to return the wrong result and fail due to a failing assertion.
