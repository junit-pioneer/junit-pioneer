...
To better react to such changes, `@ExpectedToFail` has an attribute `onExceptions` that can be used to enumerate the exceptions which when thrown will result in an aborted (and thus passing) test.
