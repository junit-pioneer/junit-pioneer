...
To create the test data, instantiate a `new CartesianProductTest.Sets()` then use the `add()` or `addAll()` methods to register the values for the parameters.
...
static CartesianProductTest.Sets nFold() {
	return new CartesianProductTest.Sets()
		// with a `List<String> letters` (or any kind of
		// `Collection` or a `Stream`, you can use `addAll`:
		// .addAll(letters)
		.add("Alpha", "Omega")
