...
This annotation might look familiar - it mimics JUnit's `@ValueSource`, except `@CartesianValueSource` is repeatable.
...
----
@CartesianProductTest
@CartesianEnumSource(TimeUnit.class)
void testWithCartesianEnumSource(TimeUnit timeUnit) {
    assertThat(timeUnit).isNotNull();
}
----

Like `@CartesianValueSource`, `@CartesianEnumSource` is an annotation that might look familiar - it mimics JUnit's `@EnumSource`, except `@CartesianEnumSource` is repeatable.
It also does NOT work with `@ParameterizedTest`.
https://junit.org/junit5/docs/current/user-guide/#writing-tests-parameterized-tests-sources-EnumSource::[You can read about `@EnumSource` here.]
...
----
@CartesianProductTest
@CartesianEnumSource(value = TimeUnit.class, names = { "DAYS", "HOURS" })
void testWithCartesianEnumSourceInclude(TimeUnit timeUnit) {
...
----
@CartesianProductTest
@CartesianEnumSource(value = TimeUnit.class, mode = EXCLUDE, names = { "DAYS", "HOURS" })
void testWithCartesianEnumSourceExclude(TimeUnit timeUnit) {
...
----
@CartesianProductTest
@CartesianEnumSource(value = TimeUnit.class, mode = MATCH_ALL, names = "^(M|N).+SECONDS$")
void testWithCartesianEnumSourceRegex(TimeUnit timeUnit) {
    assertThat(timeUnit.name()).endsWith("SECONDS");
}
----
