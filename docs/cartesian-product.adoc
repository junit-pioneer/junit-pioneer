...
`@CartesianTest.Factory` can be used to name a static factory method that supplies your arguments.
...
This method must return `CartesianMethodArgumentsProvider.Sets`.
`Sets` is a helper class, specifically for creating sets for `@CartesianTest`.
To create the test data, instantiate a `new Sets()` then use the `add()` method to register the values for the parameters.
When calling `add`, you have to pass in all the arguments you need for the corresponding parameter.
For convenience, all `add()` methods return with your `Sets` instance, so you can chain `add()` calls.
...
class MyCartesianTestClass {
	@CartesianTest
	@CartesianTest.Factory("provideArguments")
...
class BadExamples {
	@CartesianTest
	@CartesianTest.Factory("resolveParameters")
	...
	@CartesianTest
	@CartesianTest.Factory("resolveTestReporterParam")
	...
	static Sets resolveParameters() {
		return new Sets()
			.add("A", "B", "C")
			.add(1, 2, 3);
	}

	static Sets resolveTestReporterParam() {
		return new Sets()
			.add("A", "B", "C")
...
It must also provide a default (no argument) constructor.
The method `provideArguments` returns a `CartesianMethodArgumentsProvider.Sets` object.
...
class NumberArgumentsProvider implements CartesianMethodArgumentsProvider, AnnotationConsumer<NumberSource> {
	...
	public Sets provideArguments(ExtensionContext context) {
		...
		Sets sets = new Sets();
		for (int i = 0; i < paramCount; i++) {
			sets.add(Arrays.stream(numbers).boxed());
		}
		return sets;
	}
