:page-title: Clearing, Setting and Restoring Environment Variables
:page-description: The JUnit 5 (Jupiter) extensions `@ClearEnvironmentVariable`, `@SetEnvironmentVariable` and `@RestoreEnvironmentVariables` clear/set/restore the values of environment variables during a test
...
[NOTE]
====
Modifying environment variables in code requires using the Java reflection API.
The two `@RestoreEnvironmentVariables` examples uses an https://github.com/junit-pioneer/junit-pioneer/blob/63c82ed9c417048f88c334aad5678fe1148b3d4b/src/main/java/org/junitpioneer/jupiter/EnvironmentVariableUtils.java#L35[internal JUnitPioneer utility] to do that, which can be copied to your code if needed.
====

When `@RestoreEnvironmentVariables` is used on a test class, any environment variables changes made during the entire lifecycle of test class, including test methods, `@BeforeAll`, `@BeforeEach` and 'after' methods, is restored after the test class lifecycle is complete.
...
In the following example, `SomeOtherTest` is completely isolated from changes made in `MyEnvironmentVariableRestoreTest` and `isolatedTest2` is isolated from changes made in `isolatedTest1`.
Both `isolatedTest1` and `isolatedTest2` _will_ see environment variable values set in `@BeforeAll` and `@BeforeEach`.

[source,java,indent=0]
----
include::{demo}[tag=environment_class_restore]
----
