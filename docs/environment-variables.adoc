...
Note that using `@RestoreEnvironmentVariables` is not necessary to restore environment variables modified via `@ClearEnvironmentVariable` or `@SetEnvironmentVariable`.
However, it is OK and normal to use all three annotations together, which you might do if some environment values are parameterized (i.e. need to be set in code) and others are not.
