...
[source,java,indent=0]
----
include::{demo}[tag=rangesources_ranges_1, indent=0]
include::{demo}[tag=rangesources_ranges_2, indent=0]
----
...
== Usage with other `@ArgumentSource` annotations

Because of a limitation in the implementation you can not use multiple range source annotations on your test.
You _can_ use other `@ArgumentSource` annotations (e.g.: `@ValueSource`, `@MethodSource`).
