...
When `@RestoreSystemProperties` is used on a test class, any system properties changes made during the entire lifecycle of test class, including test methods, `@BeforeAll`, `@BeforeEach` and 'after' methods, is restored after the test class lifecycle is complete.
